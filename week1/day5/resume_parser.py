...
class Jobdesc(BaseModel):
    ...
    educational_qualification:list[str]
...
#convert raw json to python dictionary
import json
job_data = json.loads(raw_json)
job = Jobdesc(**job_data)
print(job.min_experience)
print(job.educational_qualification)
...
for file_path in resume_folder.iterdir():
    #week1\day5\resumes\abhay resume new - Abhay Singh.pdf
    if file_path.suffix.lower() not in [".pdf", ".docx"]:
        continue
    print("\nProcessing:", file_path.name)
    resume_text = read_resume(file_path)
    parsed_resume = parse_resume(resume_text) #llm call 1
    #sleep time to avoid rate limit
    time.sleep(5) 
